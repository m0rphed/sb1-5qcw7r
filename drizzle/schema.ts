...
import { pgTable, text, timestamp, uuid, integer, jsonb, check, primaryKey } from "drizzle-orm/pg-core";
...
// Таблица для хранения упражнений
...
export const exerciseSentences = pgTable('exercise_sentences', {
  exerciseId: uuid('exercise_id').notNull().references(() => exercises.id),
  sentenceId: uuid('sentence_id').notNull().references(() => sentences.id)
},
  (table) => ({
    pk: primaryKey({ columns: [table.exerciseId, table.sentenceId] })
  })
);
...
export const userProgress = pgTable('user_progress', {
  ...
  exerciseId: uuid('exercise_id').notNull().references(() => exercises.id),
  completionStatus: text('completion_status').default('not started').notNull(),
  lastAttempted: timestamp('last_attempted').defaultNow().notNull(),
  sentencesSkipped: jsonb('sentences_skipped').default('[]').notNull(),
...
export const userProfiles = pgTable('user_profiles', {
  ...
  completedExercisesCount: integer('completed_exercises_count').default(0).notNull(),
  sentencesByDifficulty: jsonb('sentences_by_difficulty').default('{}').notNull(),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull()
});
